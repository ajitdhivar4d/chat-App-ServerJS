...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length <= 3)
    return next(new ErrorHandler("Group must be at least 3 members", 400));
  ...
  emitEvent(req, ALERT, chat.members, {
    message: `${userThatWillBeRemoved.name} has been removed from this group`,
    chatId,
  });
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    ...
    if (!chat) return nex(new ErrorHandler("chat not found", 404));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat && !chat.members.includes(req.user.toString()))
    return next(
      new ErrorHandler(
        "You are not a member of this chat, So you don't have to authorize to delete this chat ",
        403,
      ),
    );
